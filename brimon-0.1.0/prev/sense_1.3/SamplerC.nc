...
implementation
{
  ...
  event void AccelY.bufferDone(error_t ok, uint16_t *buf, uint16_t count) {
    ...
    if(buf==buffery1) {
		...
		call BlockWriteX.write(buffersyn * sizeof avgbuffery1, avgbuffery1, sizeof avgbuffery1);
    }
    if(buf==buffery2) {
		for (i = 0; i < SUMMARY_BUFFER_SIZE; i++) {
			sum = 0;
		...
		call BlockWriteX.write(buffersyn * sizeof avgbuffery2, avgbuffery2, sizeof avgbuffery2);
    }
  }
  ...
  event void AccelZ.bufferDone(error_t ok, uint16_t *buf, uint16_t count) {
    ...
    if(buf==bufferz1) {
		...
		call BlockWriteX.write(bufferszn * sizeof avgbufferz1, avgbufferz1, sizeof avgbufferz1);
    }
    if(buf==bufferz2) {
		...
		call BlockWriteX.write(bufferszn * sizeof avgbufferz2, avgbufferz2, sizeof avgbufferz2);
    }
  }
