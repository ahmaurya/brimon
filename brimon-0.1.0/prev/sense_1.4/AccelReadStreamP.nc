...
configuration AccelReadStreamP
{
  provides {
    ...
    interface ReadStream<uint16_t> as ReadStreamZ[uint8_t client];
  }
  uses {
    interface ReadStream<uint16_t> as ActualX[uint8_t client];
    interface ReadStream<uint16_t> as ActualY[uint8_t client];
    interface ReadStream<uint16_t> as ActualZ[uint8_t client];
  }
}
implementation
{
  ...
  new ArbitratedReadStreamC(NACCEL_CLIENTS, uint16_t) as MultiplexZ;
  ...
  ReadStreamZ = MultiplexZ;
  MultiplexZ.Resource -> AccelConfigP;
  MultiplexZ.Service = ActualZ;
}
