...
implementation {
	...
	/** executed when the completion of a command execution is signalled by the component responsible for the execution.
		...
		if (locked) {
			...
			if(commType==ROUTING) {
				...
				for(i=0;i<call Routing.getNumNodes();i++) {
					rcm->payload[i] = call Routing.getParent(i+1);
				}
			}
	...
	void transfer_actual() {
		atomic if (locked) {
			...
			test_serial_msg_t* rcm = (test_serial_msg_t*)call Packet.getPayload(&packet, sizeof(test_serial_msg_t));
			if (rcm == NULL) {call Timer.startOneShot(100); return;}
			if (call Packet.maxPayloadLength() < sizeof(test_serial_msg_t)) {call Timer.startOneShot(100); return;}
	...
	event void Timer.fired() {
		if(counter*30 < (call Routing.getNumNodes()*call Sense.getLogSize()))
			transfer_actual();
		else
	...
	event void WaitTimer.fired() {
		if(commandType==TRANSFER) {
			...
			call Command.execCommand(commandType);
		}
	}
